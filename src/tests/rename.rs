...
#[test]
fn worktree_title_wins_over_a_process_title() {
...
#[test]
fn clearing_worktree_title_preserves_a_manual_title() {
    ...
    assert_eq!(tabs[0].worktree_title, None);
...
fn clearing_process_title_does_not_affect_worktree_title() {
    ...
    assert!(set_tab_name_on_tabs(tabs.iter_mut(), &request));
    assert_eq!(tabs[0].process_title, None);
    assert_eq!(tabs[0].worktree_title.as_deref(), Some("feature/api"));
}
