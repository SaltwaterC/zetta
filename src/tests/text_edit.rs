...
/// A field standing in for whichever surface owns one, so the operations can be
...
impl Field {
    ...
    fn edit(&mut self) -> TextEdit<'_> {
        TextEdit::new(&mut self.text, &mut self.cursor, &mut self.select_all)
    }
}
...
fn copying_and_cutting_take_the_whole_value_when_nothing_is_selected() {
    let mut field = Field::new("age1example");
    assert_eq!(field.edit().selected_text(), Some("age1example"));

    let mut selected = Field::new("~/keys/zetta.txt");
    selected.edit().select_all();
    assert_eq!(selected.edit().selected_text(), Some("~/keys/zetta.txt"));

    let mut empty = Field::new("");
    assert_eq!(empty.edit().selected_text(), None);
}

#[test]
fn pasting_replaces_a_selection_and_inserts_at_the_cursor_otherwise() {
    let mut field = Field::new("abcdef");
    field.cursor = 3;
    field.edit().insert("XY");
    assert_eq!(field.text, "abcXYdef");
    assert_eq!(field.cursor, 5);

    let mut selected = Field::new("abcdef");
    selected.edit().select_all();
    selected.edit().insert("XY");
