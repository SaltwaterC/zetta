...
pub(crate) fn shell_integration_help() -> String {
    ...
    let worktree_help = if cfg!(feature = "worktree") {
        "\n\nThe generated integration also provides the zwt wrapper for the standalone Git worktree command; it changes directory only after a successful operation. Worktree completion includes new, done, abort, and rerere operations, the repeatable --copy path option, and filesystem path arguments."
    } else {
        ""
    };
    format!(
