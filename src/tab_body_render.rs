...
impl Zetta {
                    ...
                    .child(
                        IconButton::new("restore-maximized-pane", IconName::Minimize)
                            ...
                            .on_click(move |_, window, cx| {
                                restore_handle
                                    .update(cx, |this, cx| {
                                        this.toggle_maximize_pane_by_id(pane_id, window, cx);
                                    })
                                    .ok();
                            }),
                    )
                    .child(
                        IconButton::new("close-maximized-pane", IconName::Close)
                            .style(ButtonStyle::Transparent)
                            .size(ButtonSize::Compact)
                            .icon_size(IconSize::XSmall)
                            .icon_color(Color::Custom(tab_colors.icon))
    ...
    fn render_minimized_pane_shelf(
        ...
        div()
            ...
            .border_t_1()
            .border_color(tab_colors.border)
            // The shelf is the bottom-most row whenever it is shown, so it
            // always owns the window's bottom corners.
            .when(rounded_bottom_left, |shelf| shelf.rounded_bl(corner_radius))
            .when(rounded_bottom_right, |shelf| {
                shelf.rounded_br(corner_radius)
            })
            .child(
                div()
                    ...
                    .flex_none()
                    .overflow_hidden()
                    .tooltip(Tooltip::text("Use the pane controls to select or restore"))
                    .child(
                        Label::new(format!("Minimized {}/{}", index + 1, count))
                            .size(LabelSize::Small)
                            .color(Color::Custom(tab_colors.text_muted))
                            .line_clamp(1),
                    ),
            ...
            .when(count > 1, |shelf| {
                shelf.child(
                    IconButton::new("previous-minimized-pane", IconName::ChevronLeft)
                        .style(ButtonStyle::Transparent)
                        .size(ButtonSize::Compact)
                        .icon_size(IconSize::XSmall)
                        .icon_color(Color::Custom(tab_colors.icon))
