...
ifneq ($(LINUX_USER_INSTALL),)
	...
	desktop_entry="$(LINUX_USER_DESKTOP_DIR)/$(APP_ID).desktop"; desktop_source="resources/linux/$(APP_ID).desktop"; if test -f "$$desktop_entry"; then if grep -Fqx "# ZETTA MANAGED PROFILE ACTIONS BEGIN" "$$desktop_entry"; then if grep -Fqx "# ZETTA MANAGED PROFILE GROUPS BEGIN" "$$desktop_entry"; then desktop_source="$$desktop_entry"; fi; fi; fi; desktop_tmp="$$desktop_entry.tmp.$$$$"; if sed -e "s|^TryExec=.*|TryExec=$(BINDIR)/zetta|" -e "s|^Exec=zetta --new-window$$|Exec=$(BINDIR)/zetta --new-window|" -e "s|^Icon=.*|Icon=$(ICON_512_DIR)/$(APP_ID).png|" "$$desktop_source" > "$$desktop_tmp"; then if chmod 644 "$$desktop_tmp"; then if mv -f "$$desktop_tmp" "$$desktop_entry"; then :; else rm -f "$$desktop_tmp"; exit 1; fi; else rm -f "$$desktop_tmp"; exit 1; fi; else rm -f "$$desktop_tmp"; exit 1; fi
endif
