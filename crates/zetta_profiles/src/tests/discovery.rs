...
fn homebrew_shells_are_profiles_with_their_installed_program_paths() {
    ...
    assert_eq!(
        profiles[0].command,
        ProfileCommand::program(bin.join("bash").to_string_lossy().into_owned())
    );
    assert_eq!(profiles[1].name, "Fish (Homebrew)");
    assert_eq!(
        profiles[1].command,
        ProfileCommand::program(bin.join("fish").to_string_lossy().into_owned())
    );
}
...
fn path_resolved_homebrew_shells_match_direct_discovery() {
    ...
    for program in ["bash", "fish"] {
        ...
        let direct_profile = direct_profiles
            .iter()
            .find(|profile| {
                profile.command == ProfileCommand::program(path.to_string_lossy().into_owned())
            })
            .cloned()
            .unwrap();
...
fn creates_a_profile_for_each_wsl_distribution() {
    ...
    assert_eq!(
        profiles[0].command,
        ProfileCommand::with_args(
            "wsl.exe",
            vec!["--distribution".to_owned(), "Ubuntu".to_owned()]
        )
    );
}
...
fn creates_msys2_profiles_for_installed_shells_using_the_launcher() {
    ...
    for (profile, shell) in profiles.iter().zip(["bash", "zsh"]) {
        ...
        assert!(args[3].contains("-defterm -here -no-start -msys -use-full-path"));
        assert!(args[3].ends_with(&format!("-shell {shell}\"")));
    }
}
...
fn creates_cygwin_profiles_for_installed_shells_with_direct_commands() {
    ...
    for (profile, shell) in profiles.iter().zip(["bash", "zsh", "fish", "nu"]) {
        assert_eq!(
            profile.command,
            ProfileCommand::with_args(
                root.path()
...
fn msys2_launcher_command_supports_custom_paths_with_spaces() {
    ...
    let program = profile
        .command
        .program
        .expect("an MSYS2 profile runs the launcher through cmd.exe");
    let args = profile.command.args;
