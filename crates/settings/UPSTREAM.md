...
This crate is synchronized with `zed/crates/settings` at Zed revision
...
- `publish` is `false`.
- the `rust_embed` `#[folder]` attribute points at `../../zed/assets`,
  because it is resolved relative to the manifest and the asset directory did
