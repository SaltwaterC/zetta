# Zetta release_channel fork

This crate is synchronized with `zed/crates/release_channel` at Zed revision
...
- `publish` is `false`.
- the `include_str!` for `RELEASE_CHANNEL` points at
  `../../../zed/crates/zed/RELEASE_CHANNEL`.
