...
impl Terminal {
            ...
            TerminalBackendEvent::Title(title) => {
                if self.init_command_startup_done_title.as_deref() == Some(title.as_str()) {
                    ...
                    if self.init_command_startup_suppress_render {
                        ...
                        // The wrapper can be echoed by zsh's line editor before
                        // `stty -echo` runs. Clear it only after the wrapper has
                        // restored echo, so late output from that command cannot
                        // leak into the first visible frame.
                        clear_saved_screen(&mut terminal);
                    }
    ...
    fn cancel_init_command_startup(&mut self) {
        ...
        // before the queued keystrokes that raced the handshake.
        if self.init_command_startup_uses_nul {
            self.write_to_pty(b"\0");
        } else {
        ...
        if self.init_command_startup_done_title.is_none() {
            self.release_init_command_startup_render();
        }
        self.init_command_startup_uses_nul = false;
    }
...
mod tests {
    ...
    async fn startup_render_cancellation_discards_internal_command(cx: &mut TestAppContext) {
        ...
        // Output generated by the wrapper can arrive after cancellation. It
        // must stay hidden until the wrapper reports that echo is restored.
        terminal.update(window, |terminal, cx| {
            terminal.write_output(
                b"late startup wrapper\n__zed_init_command_ready_4244__\nprompt",
                cx,
            );
        });
        window.update_window_entity(&terminal, |terminal, window, cx| {
        ...
        terminal.update(window, |terminal, cx| {
            terminal.process_event(
                TerminalBackendEvent::Title(init_command_startup_done_title(4244)),
                cx,
            );
        });
        window.update_window_entity(&terminal, |terminal, window, cx| {
        ...
        let content = terminal.update(window, |terminal, _| terminal.get_content());
        assert!(!content.contains("late startup wrapper"));
        assert!(!content.contains("__zed_init_command_ready_4244__"));
        assert!(content.contains("prompt"));
        let input_log = terminal.update(window, |terminal, _| terminal.take_input_log());
        assert_eq!(input_log, vec![b"user input".to_vec()]);
        let pty_write_log = terminal.update(window, |terminal, _| terminal.take_pty_write_log());
        assert_eq!(pty_write_log, vec![b"\r".to_vec(), b"user input".to_vec()]);
    }
