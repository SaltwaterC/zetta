...
mod tests {
    #[test]
    fn rep_defaults_chunks_attributes_and_wrap() {
        let mut parser = crate::Parser::new(3, 8, 0);
        parser.process(b"\x1b[b");
        assert_eq!(parser.screen().cursor_position(), (0, 0));
        parser.process(b"A\x1b[");
        parser.process(b"0b\x1b[b\x1b[31m\x1b[7b");
        assert_eq!(parser.screen().contents(), "AAAAAAAAAA");
        assert_eq!(parser.screen().cursor_position(), (1, 2));
        assert_eq!(
            parser.screen().cell(0, 3).unwrap().fgcolor(),
            crate::Color::Idx(1)
        );
        parser.process(b"\x1bc\x1b[4b");
        assert_eq!(parser.screen().contents(), "");
    }

    #[test]
    fn rep_survives_screen_clone_and_repeats_unicode() {
        let mut parser = crate::Parser::new(3, 20, 0);
        parser.process("界".as_bytes());
        let mut restored = crate::Parser::new(3, 20, 0);
        *restored.screen_mut() = parser.screen().clone();
        restored.process(b"\x1b[2b");
        assert_eq!(restored.screen().contents(), "界界界");
        assert_eq!(restored.screen().cursor_position(), (0, 6));
    }
}
