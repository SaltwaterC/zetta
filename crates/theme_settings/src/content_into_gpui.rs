//! Conversions from `settings_content` value types into their gpui
//! equivalents.
//!
//! Vendored from Zed's `settings` crate, which Zetta no longer depends on:
//! it was the settings *store* Zetta dropped, and this trait travelled with
//! it even though it only touches `settings_content` and gpui types.
